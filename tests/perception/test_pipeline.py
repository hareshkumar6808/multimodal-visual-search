...
class DeterministicMockOCR(OCREngine):
    ...
    def extract(self, image_bytes: bytes) -> OCRResult:
        return OCRResult(
            ...
            regions=[
                {"bbox": {"x": 10, "y": 10, "width": 100, "height": 20}, "text": self.text, "confidence": self.conf}
            ],
        )


def make_png(width: int = 400, height: int = 200, color: tuple[int, int, int] = (255, 255, 255)) -> bytes:
