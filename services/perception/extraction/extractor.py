    ...
    if primary_modality == "code":
        ...
        for line in lines:
            ...
            if last_line_bottom is not None and (line_top - last_line_bottom) > (line_height * 2):
                ...
                if current_block:
                    block_bboxes = [
                        region.get("bbox", {}) for line in current_block for region in line
                    ]
                    block_text = "\n".join(
                        " ".join(region.get("text", "") for region in line)
        ...
        if current_block:
            block_bboxes = [region.get("bbox", {}) for line in current_block for region in line]
            block_text = "\n".join(
                " ".join(region.get("text", "") for region in line) for line in current_block
            )
            extracted.append(
                ExtractedObject(
                    id=f"obj_{obj_counter}",
