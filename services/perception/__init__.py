...
def health() -> dict[str, Any]:
    """Report whether the configured real OCR pipeline is ready without exposing paths."""
    try:
        pipeline = get_pipeline()
    except OCREngineUnavailableError as exc:
        return {"available": False, "ocr": "unavailable", "message": str(exc)}
    return {
        "available": pipeline.ocr_engine.is_available(),
        "ocr": type(pipeline.ocr_engine).__name__,
    }
